...
function load (Application) {
  ...
  const onRendererBegin = event => {
    ...
    for (const urlMapping of event.urls) {
      ...
      const kindString = ReflectionKind.singularString(urlMapping.model.kind)

      if (!MODELS.includes(kindString)) {
        continue
      }
      ...
      const originalName = urlMapping.model.getFriendlyFullName()
      ...
      if (typedocs[context.manifestPath].typedocs[originalName] != null) {
        Application.logger.warn(`Duplicate exported type name ${originalName} defined in ${urlMapping.model.sources[0].fullFileName}`)
      } else {
        // store reference to generate doc url
        typedocs[context.manifestPath].typedocs[originalName] = `${ghPages}${urlMapping.url}`
      }
    }
